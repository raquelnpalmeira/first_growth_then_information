%% Division function
function[daughter1_aa,daughter2_aa,daughter1_n,daughter2_n,daughter1_s,daughter2_s] = div_fun(mother_aa,mother_n,mother_s,mu,sigma)
 ...
 %% FOR SIZE
        ...
        % sample proportions that will go into each daughter cell
        for_cell_1_s = random(t_pd, 1,1);
        for_cell_2_s = 1 - for_cell_1_s;


        % assign polymers to each daughter cell
        daughter1_s = mother_s;
        daughter2_s = mother_s;
        c  = round(mother_s.*for_cell_1_s);
        d = round(mother_s.*for_cell_2_s);

        if sum(c+d ~= mother_s)>0
            error('Sum of daughter cell fatty acids more than molecules in mother cell')
        end

	daughter1_s = c;
        daughter2_s = d;

        if sum(daughter1_s+daughter2_s ~= mother_s, 'all')
            error('Sum of daughter cell fatty acids  more than molecules in mother cell')
        end
