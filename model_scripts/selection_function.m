...
function [n_m_mat, div_per_gen, n_pops, aa_pops, s_pops,  k_avg_vec, avg_length_long_vec, N_m_vec_vec, p_c_vec_vec] = selection_function_test(n, n_gen, AA_min, N_min, S_min,  p_p_min, p_c_min, p_t_min,p_d,cat_c, cat_p, w,gamma, gamma2, epsilon, alpha, beta, er, min_length_for_avg)
...
for g = 1:n_gen
    ...
    for j = 1:n
        [n_pop_end{j}, aa_pop_end{j},s_pop_end{j}, k_avg(j), total_mon_n, total_n_free_mon, total_aa_free_mon, total_n_mol,total_aa_mol,long_phobic, long_phylic, N_m_vec(j), p_c_vec(j), avg_length_long(j), num_pol_long] = model_fun(n_pop{j},aa_pop{j},s_pop{j},alpha, beta, gamma, gamma2, epsilon,p_p_min,p_c_min,p_t_min,p_d,cat_c, cat_p,N_min, AA_min,S_min,p_m, min_length_for_avg,t_mats_c,t_mats_t,t,w);
    end
    ...
    %Use the variable S for cell division
	...
	if rem(g, 500) == 0
    		sizes = cellfun(@sum, s_pop_end);
    		td = find(sizes >= threshold_size);
    	if ~isempty(td)
        	fprintf('gen %d | dividing: [%s]\n', g, num2str(td));
    		else
       	 fprintf('gen %d | no division\n', g);
    		end
	end
    ...
    n_pop = n_pop_end;
    s_pop = s_pop_end;
    div_per_gen(1,g) = sum(cell2mat(s_pop_end)>=threshold_size);
    
if div_per_gen (1,g) >0
to_divide = find(cell2mat(s_pop_end)>=threshold_size);
...
d1_s = cell(1,length(to_divide));
d2_s = cell(1,length(to_divide));

for i = 1:length(to_divide)

    [d1_aa{1,i},d2_aa{1,i},d1_n{1,i},d2_n{1,i},d1_s{1,i},d2_s{1,i}] = div_fun(aa_pop_end{1,to_divide(i)},n_pop_end{1,to_divide(i)},s_pop_end{1,to_divide(i)},0.5,0.2);
...
s_pop{to_divide(j)}=d1_s{j};
end


% Delete other random cells and replace with daughter 2
