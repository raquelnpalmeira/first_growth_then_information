function [N,P,S, k_avg, total_mon_n, total_n_free_mon, total_aa_free_mon, total_n_mol,total_aa_mol, long_phobic, long_phylic, N_m_vec, p_c_vec, avg_length_long, num_pol_long] = model_fun(N,P,S,alpha,beta, gamma, gamma2, epsilon,p_p_min,p_c_min,p_t_min,p_d,cat_c, cat_p, N_min, AA_min, S_min, p_m, min_length_for_avg,t_mats_c,t_mats_t, t_max,w)
...
if cat_c == 1 % If assuming hydrophobic peptides catalyse carbon fixation
    ...
    S =	binornd(S_min,	p_m_cat);
elseif cat_c == 0
    N_m = binornd(N_min,p_m);%N_min*p_m;
    AA_m = binornd(AA_min,p_m);%AA_min*p_m;
elseif cat_c == 2 % If hydrophobic peptides catalyse size but not monomer additions
    p_m_cat = p_m + (p_m*k_c_total);
    if p_m_cat>1
        p_m_cat = 1;
    end
    N_m = binornd(N_min, 10*p_m);
    AA_m = binornd(AA_min,10*p_m);
    S = S + binornd(S_min, p_m_cat);
else 
